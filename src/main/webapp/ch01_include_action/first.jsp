   ...
   /*
      ...
      * flush : jsp 페이지는 기본적으로 페이지의 내용을 버퍼에 저장한다.
      ...
      2. flush="false"
         ...
         버퍼에 그대로 쌓아둔 채로 자식에게 제어가 이동된다. 자식의 내용이 모두 처리가 되면
         자식의 html 내용도 버퍼에 저장한다. 그리고 제어가 부모에게로 돌아온다.
         돌아와서 부모의 나머지 부분을 처리하고 그 부분도 버퍼에 쌓아둔다. 최종적으로 부모와
         자식의 html이 모두 버퍼에 쌓이면 그 때 flush한다.
         
      [2] @ include file="포함할 파일" : include 디렉티브(지시자)
      
      1. 다른 페이지를 포함하는 데 컴파일 타임에 포함될 자식 페이지의 내용을 부모에게 포함시킨다.
